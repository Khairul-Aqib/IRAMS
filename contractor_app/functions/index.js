...
// =============================================================================
// Support Chat Notifications — Fired when a new message is added
// =============================================================================

exports.onSupportMessageCreated = onDocumentCreated('SupportChats/{chatId}/messages/{messageId}', async (event) => {
  ...
  // 1. Only notify if the message is from the Admin/Customer
  if (!messageData.isFromAdmin) {
    console.log(`onSupportMessageCreated: Message is from contractor, skipping.`);
    return;
  }

  // 2. Get the Chat Document to find out WHICH contractor this is
  const chatDoc = await db.collection('SupportChats').doc(chatId).get();
  if (!chatDoc.exists) return;
  
  const chatData = chatDoc.data();
  // Using your hybrid schema!
  const contractorId = chatData.userId || chatData.ContractorId; 
  const senderName = chatData.userName || 'Customer Support';

  if (!contractorId) {
    console.log(`onSupportMessageCreated: No contractor ID found in chat doc.`);
    return;
  }

  // 3. Go to the Contractors collection and get their FCM Token
  const contractorDoc = await db.collection('Contractor').doc(contractorId).get();
  if (!contractorDoc.exists) return;

  const contractorData = contractorDoc.data();
  // Checking both PascalCase and camelCase just to be safe
  const pushToken = contractorData.FcmToken || contractorData.fcmToken; 

  if (typeof pushToken !== 'string' || pushToken.trim().length === 0) {
    console.log(`onSupportMessageCreated: Contractor ${contractorId} has no FCM token saved.`);
    return;
  }
  ...
  try {
    ...
    console.log(`Support FCM sent successfully to contractor: ${contractorId}`);
  } catch (err) {
    console.error(`Support FCM send failed for ${contractorId}:`, err.code || err.message);
    
    // Auto-clean dead tokens just like your sendToUser function
    if (
      err.code === 'messaging/registration-token-not-registered' || 
      err.code === 'messaging/invalid-registration-token'
    ) {
      try {
        await contractorDoc.ref.update({ FcmToken: FieldValue.delete() });
        console.log(`Cleaned up dead token for contractor: ${contractorId}`);
      } catch (_) {}
    }
  }
});
...
// =============================================================================
// Admin Management — Create New Admin/Manager Accounts (Managers Only)
// =============================================================================

exports.createAdminAccount = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'You must be logged in to create an admin.');
  }

  const managerUid = request.auth.uid;
  const { name, email, password, ic, phone, role } = request.data;

  // 1. Query the database
  const managerQuery = await db.collection('Admins').where('uid', '==', managerUid).get();
  
  // 2. DIAGNOSTIC CHECK 1: Did it find the document?
  if (managerQuery.empty) {
    // This will print the EXACT UID the server thinks you are logged in as
    throw new HttpsError('permission-denied', `DEBUG FAIL 1: No doc found for UID [${managerUid}]`);
  }

  const managerData = managerQuery.docs[0].data();
  const actualRole = managerData.Role;

  // 3. DIAGNOSTIC CHECK 2: Is the role exactly "Manager"?
  if (actualRole !== 'Manager') {
    // This will reveal if there is a hidden space like "Manager "
    throw new HttpsError('permission-denied', `DEBUG FAIL 2: Found doc, but Role is [${actualRole}]`);
  }

  const managerName = managerData.AdminName;
  ...
  try {
    ...
    await db.collection('Admins').doc(userRecord.uid).set({
      ...
      AdminStatus: "Active",
      Role: role,
      AccountCreatedByWhichManager: managerName,
      CreatedWhen: FieldValue.serverTimestamp(),
      uid: userRecord.uid
    });

    return { success: true, message: "Admin account created successfully!", uid: userRecord.uid };
  } catch (error) {
...
// =============================================================================
// Update Admin Profile (Manager Only) - Including Picture Fix
// =============================================================================
...
exports.updateAdminProfile = onCall(async (request) => {
  ...
  // 1. We must extract AdminPicture from the incoming data!
  const { 
    targetUid, 
    AdminName, 
    AdminEmail, 
    ...
    AdminIC, 
    AdminStatus,
    AdminPicture // <--- ADDED THIS
  } = request.data;

  const managerQuery = await db.collection('Admins').where('uid', '==', request.auth.uid).get();
  if (managerQuery.empty || managerQuery.docs[0].data().Role !== 'Manager') {
    throw new HttpsError('permission-denied', 'Only Managers can update admin profiles.');
  }

  try {
    await require('firebase-admin/auth').getAuth().updateUser(targetUid, {
      email: AdminEmail,
      displayName: AdminName
    });

    // 2. We must explicitly tell Firestore to save the AdminPicture URL
    await db.collection('Admins').doc(targetUid).update({
      ...
      AdminPicture: AdminPicture // <--- ADDED THIS
    });
